...
- `studio/modals/_host.html.erb` — **modal stack host (v0.4.5+)**. Registers `Alpine.store('modals')` — a LIFO stack with `open(id, props)` / `close()` / `closeAll()` / `current()` / `isOpen(id)`. Multiple modals can be open simultaneously and render Z-indexed. Each modal's `props` persists across re-renders (good for multi-step flows). `dismissible: false` blocks Escape + click-outside (critical for on-chain TX flows that must not be orphaned by an accidental dismiss). Modals auto-clear on bfcache `pageshow` and `turbo:before-cache`. The helper `window.StudioModals.holdAtLeast(minMs)` returns a thenable to enforce a minimum spinner duration on fast operations. Ships 4 reusable cards (processing, success, error, confirm). **Critical:** each modal partial mounted via `<template x-if="$store.modals.current()?.id === 'your-id'">` must render a SINGLE root element — siblings are silently dropped. Apps override the host only to register app-specific modal partials.
- **Image upload (v0.4.12)** — `studio/_cropper_assets` (cropper.js + the inline factories `imageUploadHost` / `submitFormWithProgress` / `cropPhotoModal`) plus the `studio/modals/_crop_photo` and `studio/modals/_saving` modal partials. Add a cropped image upload (avatar, banner, logo) with one `x-data="imageUploadHost({ aspectRatio:, maxWidth:, filename:, saving:, … })"`, `render "studio/cropper_assets"` on the page, and register the two modals in your host **block**. **Not** auto-registered by the host: the JS ships with `cropper_assets` (page-scoped) so it survives a consumer overriding `studio/modals/_host` — which Turf Monster does substantially (the `swap()`/`advance()` animated host). See CHANGELOG v0.4.12 → Integration. **v0.4.13** adds the deferred-form-field counterpart: `components/_avatar_cropper` (stages a cropped file on a hidden input + round preview; the enclosing signup/profile form submits later) now drives through the same shared `crop-photo` modal.
